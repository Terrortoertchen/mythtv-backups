...
export PATH=/home/terror/.linjark/bin:/opt/apache-maven-3.3.9/bin:$PATH

if [ "$PS1" ] ; then  
   sudo mkdir -p -m 0700 /dev/cgroup/cpu/user/$$ > /dev/null 2>&1
   echo $$ |sudo tee /dev/cgroup/cpu/user/$$/tasks
   echo "1" |sudo tee /dev/cgroup/cpu/user/$$/notify_on_release
fi
